...
// 👉 영문 구 키를 한글 구 이름으로 정확하게 변환해 주는 사전
const guNameMap = {
...
export async function generateMetadata({ params }) {
  ...
  return {
    ...
    description: `경기도 ${cityName} ${guName} 출장마사지 전문. 25분 내 신속 방문, 24시 친절 상담 및 후불제 홈케어 서비스.`,
  };
}
...
export default async function GuPage({ params }) {
  ...
  const cityName = gyeonggiDistricts[districtKey]?.name || '경기도';
  const guName = guNameMap[guKey] || guKey; // 'yeongtong'이 들어오면 '영통구'로 자동 변환됨
      ...
      <main className="max-w-4xl mx-auto px-4 py-8 w-full flex-1">
        <section className="text-center my-6">
          <h1 className="text-3xl md:text-5xl font-bold text-white mb-3">
            <span className="text-amber-400">경기도 {cityName} {guName}</span><br />24시 출장마사지 추천
          </h1>
          <p className="text-gray-400 text-sm md:text-base mb-4">{cityName} {guName} 전지역 오피스텔 및 자택 25분 내 신속 방문!</p>
        </section>
            ...
            >
              <div className="flex justify-between items-start mb-3">
                <div>
                  ...
                  <p className="text-xs text-amber-400 mt-1">{cityName} {guName} 및 인근 전지역</p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <a
                  href={`sms:${shop.phone}?body=${encodeURIComponent(`[${cityName} ${guName} 출장마사지] ${shop.name} 문의드립니다.`)}`}
