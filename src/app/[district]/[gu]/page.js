// ⭐️ 경기도 주요 시/구 영문-한글 매핑 객체
const gyeonggiDistricts = {
  suwon: { 
    name: '수원시', 
    gus: { jangan: '장안구', gwonseon: '권선구', paldal: '팔달구', yeongtong: '영통구' }
  },
  seongnam: { 
    name: '성남시', 
    gus: { sujeong: '수정구', jungwon: '중원구', bundang: '분당구' }
  },
  goyang: { 
    name: '고양시', 
    gus: { deogyang: '덕양구', ilsandong: '일산동구', ilsanseo: '일산서구' }
  },
  yongin: { 
    name: '용인시', 
    gus: { cheoin: '처인구', giheung: '기흥구', suji: '수지구' }
  },
  bucheon: { 
    name: '부천시', 
    gus: { wonmi: '원미구', sosa: '소사구', ojeong: '오정구' }
  },
  ansan: { 
    name: '안산시', 
    gus: { sangrok: '상록구', danwon: '단원구' }
  },
  anyang: { 
    name: '안양시', 
    gus: { manan: '만안구', dongan: '동안구' }
  }
};
...
// ⭐️ [핵심] 네이버/구글 검색로봇용 한글 메타데이터 생성 함수
export async function generateMetadata({ params }) {
  const { district, gu } = await params;
  
  const districtInfo = gyeonggiDistricts[district];
  const cityName = districtInfo?.name || district; // 예: 부천시
  const guName = districtInfo?.gus?.[gu] || gu;    // 예: 소사구

  const locationTitle = `${cityName} ${guName}`;

  return {
    title: `경기도 ${locationTitle} 출장마사지 24시 홈케어 추천 | 경기건마사랑`,
    description: `경기도 ${locationTitle} 전지역 25분 내 빠른 방문! 타이, 아로마, 스웨디시 24시 후불제 출장마사지.`,
    openGraph: {
      title: `경기도 ${locationTitle} 출장마사지`,
      description: `경기도 ${locationTitle} 24시 안심 출장마사지 서비스.`,
    },
  };
}

// ⭐️ 메인 화면 컴포넌트 (서버 컴포넌트)
          ...
          {shops.map((shop) => (
            <article key={shop.id} className="bg-[#141414] border border-white/10 rounded-2xl p-5 md:p-6 shadow-lg hover:border-amber-500/40 transition-all">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <a href={`sms:${shop.phone}?body=${encodeURIComponent(`[${cityName} ${guName} 출장마사지] ${shop.name} 문의드립니다.`)}`} className="flex items-center justify-center gap-1.5 bg-white/10 hover:bg-white/20 text-white font-bold py-3 rounded-xl text-xs border border-white/10 transition-colors">
                  💬 문자 예약하기
                </a>
              </div>
            </article>
